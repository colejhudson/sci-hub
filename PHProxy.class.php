   ...
   (
    ...
    function PHProxy($config)
    {
		...
		if (isset($_SESSION['cookies']))
			$this->all_cookies = $_SESSION['cookies'];
		else
			$this->all_cookies = array();
		
		$this->proxies = array();
		$proxy_classes = scandir(dirname(__FILE__).'/proxies');
		foreach ($proxy_classes as $p)
			if (preg_match("/(.*?).class.php/",$p,$name))
			{
				$proxy_class = $name[1];
				require_once(dirname(__FILE__).'/proxies/'.$p);
				$this->proxies[] = new $proxy_class();
			}
			
		$this->using_proxy = false;
		$this->current_proxy = false;
		$this->proxy_dead = false;
		
		$this->unsupported_host = false;
		
		$this->transfer_started = false;
		$this->redirect = false;
		...
		if (!isset($_SESSION['requested_urls']))
			$_SESSION['requested_urls'] = array();
	...
	{
		if (strlen($this->response_body) < 16384 && (strtolower($this->content_type)!='application/pdf'))
			return;
		if ($this->request_method=='POST')
			return;
		foreach ($GLOBALS['exclude_log'] as $exclude)
			if (strpos($this->url,$exclude)!==false)
				return;
		
		$ignore = array('css','jpg','gif','png','js');
		$url_parts = explode('?',$this->url);
		$url_parts = explode('.',$url_parts[0]);
		if (in_array($url_parts[count($url_parts)-1],$ignore))
			return;
		
		$proxy = '';
		if ($this->using_proxy)
			$proxy = $this->current_proxy->name;
		$type = '';
		if (strtolower($this->content_type)=='application/pdf')
			$type = 'PDF';

		//$url = mysql_real_escape_string($this->url);
		//$u_agent = mysql_real_escape_string($_SERVER['HTTP_USER_AGENT']);
		//mysql_query("INSERT INTO log(client_ip,request,proxy,`type`,note) VALUES ('".get_user_ip()."','".$url."','".$proxy."','".$type."','".$u_agent.$note."')");
		
		$this->logger->add(array(get_user_ip(),$this->url,$proxy,$type,$_SERVER['HTTP_USER_AGENT'].$note));

		if ($this->request_method!='POST')
		{
			if (!isset($_SESSION['requested_urls'][$this->url]))
				$_SESSION['requested_urls'][$this->url] = array();
			if (in_array($proxy,$_SESSION['requested_urls'][$this->url]))
			{
				$_SESSION['requested_urls'][$this->url] = array_diff($_SESSION['requested_urls'][$this->url], array($proxy));
				$_SESSION['requested_urls'][$this->url] = array_values($_SESSION['requested_urls'][$this->url]);
			}
			$_SESSION['requested_urls'][$this->url][] = $proxy;
		}
		
		
		if (isset($_SESSION['last_request']))
		{
			if (md5($this->url)==$_SESSION['last_request']['url'])
				$_SESSION['last_request']['count']++;
			else
			{
				$_SESSION['last_request']['url'] = md5($this->url);
				$_SESSION['last_request']['count'] = 1;
			}
		}
		else
		{
    ...
    function start_transfer($url)
    {
		...
		if ($this->transfer_started && $this->using_proxy)
			$url = encode_url(str_replace($this->current_proxy->domain,'',decode_url($url)));

        $this->set_url($url);
		
		log_event_forced('requested '.$this->url);
		if ($this->transfer_started) log_event_forced('transfer started already');
		
		if ($this->base['host']=='onlinelibrary.wiley.com' && strpos($this->url,'.pdf?')!==false)
		{
			$params_tmp = explode('&',$this->base['query']);
			$params = array();
			foreach ($params_tmp as $param)
			{
				$p_key = explode('=',$param);
				$params[$p_key[0]] = $p_key[1];
			}
			if (isset($params['v']) && isset($params['t']) && isset($params['s']))
			{
				$this->unsupported_host = true;
				
				log_event_forced('WILEY!!!!');
				
				$this->content_type = 'application/pdf';
				$this->log_request(' ----> REDIRECTED TO WILEY');
				
				return;
			}
		}
		
		$this->doi = ($this->base['host']=='dx.doi.org');
		
		if (isset($_SESSION['last_request']))
			if ($_SESSION['last_request']['count']>5 && md5($this->url)==$_SESSION['last_request']['url'])
			{
				$this->attack = true;
				return;
			}
		
		if (!$this->transfer_started || $this->doi)
		{
			log_event("looking for proxy...");
			
			$proxies_good = $this->database->domain_check($this->url_segments['host']);
			
			if (count($proxies_good)==0)
			{
				$this->unsupported_host = true;
				log_event("domain not supported");
				return;
			}
			...
			{
				$proxy_queue = array();
				foreach ($this->proxies as $i => $p)
					if (in_array($p->name,$_SESSION['requested_urls'][$this->url]))
					{
						$proxy_queue[$p->name] = $p;
						unset($this->proxies[$i]);
					}
				$this->proxies = array_values($this->proxies);
				foreach ($_SESSION['requested_urls'][$this->url] as $p_name)
					if (isset($proxy_queue[$p_name]))
						$this->proxies[] = $proxy_queue[$p_name];
			}
			
			$ref = (isset($_GET['ref']))?$_GET['ref']:'';
			if (strpos($ref,' addendum')!==false) $ref = substr($ref,0,strpos($ref,' addendum'));
			//if (strlen($ref)>0 && !in_array($ref,$proxies_good))
			//	$proxies_good[] = $ref;
			
			foreach ($this->proxies as $p)
				if (in_array($p->name,$proxies_good) && ($ref=='' || $ref==$p->name))
				{
					if (!$p->ready())
						continue;
				
					$this->current_proxy = $p;
					
					$my_url = str_replace($this->url_segments['host'],$this->url_segments['host'].$this->current_proxy->domain,$this->url);
					$this->proxy_cookies = $this->current_proxy->get_in($my_url);
					
					if ($this->proxy_cookies!==false)
						break;
					else
						$this->current_proxy = false;
				}
			if ($this->current_proxy != false)
			{
				$this->using_proxy = true;
				log_event("selected proxy ".$this->current_proxy->name);
			}
			else
			{
				$this->proxy_dead = true;
				log_event("no active proxies");
				return;
			}
		}
		...
		if ($this->using_proxy)
		{
			if (count(explode('.',$this->url_segments['host']))==2 && !$this->transfer_started)
			{
				$this->url = str_replace($this->url_segments['host'],'www.'.$this->url_segments['host'],$this->url);
				$this->url_segments['host'] = 'www.'.$this->url_segments['host'];
			}
			$this->url_segments['host'] .= $this->current_proxy->domain;
		}
		
		
		$cache_i = md5($this->url.$this->current_proxy->name);
		
		if (isset($_SESSION['cache'][$cache_i]) && $this->request_method!='POST')
		{
			$this->response_body = gzuncompress($_SESSION['cache'][$cache_i][0]);
			$this->response_headers = $_SESSION['cache'][$cache_i][1];
			...
			foreach ($GLOBALS['noscript_hosts'] as $h)
				if (strpos($this->url_segments['host'],$h)!==false)
					$this->flags['remove_scripts'] = 1;
			foreach ($GLOBALS['script_hosts'] as $h)
				if (strpos($this->url_segments['host'],$h)!==false)
					$this->flags['remove_scripts'] = 0;
			
			return;
		}
		
		
		$this->transfer_started = true;
		
		log_event("transfer started");		
		...
		//log_event(got response",$this->response_body);
		log_event("got response");
		...
		if ($this->using_proxy)
		{
			if (strpos($this->response_headers,"utf-8") === false)
				$domain = iconv("UTF-8", "CP1251", $this->current_proxy->domain);
			else
				$domain = $this->current_proxy->domain;
			$this->response_body = str_replace($domain, "", $this->response_body);
			$this->url_segments['host'] = str_replace($this->current_proxy->domain,'',$this->url_segments['host']);
			
			
			if ($this->request_method!='POST')
			{
				$_SESSION['cache'][$cache_i] = array();
				$_SESSION['cache'][$cache_i][0] = gzcompress($this->response_body);
				$_SESSION['cache'][$cache_i][1] = $this->response_headers;
			}
	...
	log_event('sending request...',$this->request_headers);
		...
		//log_event('server reply',$this->response_headers);
		
		if (strpos(strtolower($this->response_headers),'404 not found')!==false)
		{
			$this->response_body = '404 Not Found';
			log_event("not found");
			return;
		}
		...
		$redirect = $this->follow_location();
		
        if ($redirect && $this->max_redirect>0 && $this->url_prev!=$this->url)
        {
            fclose($this->socket);

				log_event_forced("redirect");
        ...
        else
        {
			...
			if ($redirect)
				 $this->url = decode_url($this->url);
			
            if (!in_array($this->content_type, array('text/html', 'application/xml+xhtml', 'application/xhtml+xml', 'text/css', 'application/x-javascript', 'text/javascript')) && (!$this->content_length || (int)$this->content_length <= $this->config['max_file_size']))
            {
				log_event("non-hypertext mime type");
...
//            'embed'      => array('src'),
		...
		//log_event('url',$this->url);
		//log_event('found tags',implode(", ",$matches[1]));
		//log_event('response',$this->response_body);
		
        for ($i = 0, $count_i = count($matches[0]); $i < $count_i; $i++)
        {
            $tag = strtolower($matches[1][$i]);

            if (  (!isset($tags[$tag]) || !preg_match_all("#([a-zA-Z\-\/]+)\s*(?:=\s*(?:\"([^\">]*)\"?|'([^'>]*)'?|([^'\"\s]*)))?#", $matches[2][$i], $m, PREG_SET_ORDER)) && !in_array($tag,$special_tags)    )
            {
                continue;
            }
            ...
            switch ($tag)
            {
                ...
                case 'form':
                    if (isset($attrs['action']))
                    {
						...
						if (strpos($attrs['action'],"http://") === false)
						{
							if ($attrs['action'][0]!='/')
								$attrs['action'] = '/'.$attrs['action'];
							$attrs['action'] = "http://".$this->url_segments['host'].$attrs['action'];
						}
				...
				case 'a':
					...
					if (isset($attrs['href']) && strlen($attrs['href'])>0)
						$attrs['href'] .= '&ref=' . urlencode($this->current_proxy->name) . '+addendum';
					
					if (!isset($attrs['target']) || ($attrs['target']!='_blank' && $attrs['target']!='_new' && $attrs['target']!='newPdfWin'))
				...
				case 'frame':
					...
					if (isset($attrs['src']) && strlen($attrs['src'])>0)
						$attrs['src'] .= '&ref=' . urlencode($this->current_proxy->name);
    ...
    function set_request_headers()
    {
        ...
        $headers  = "{$this->request_method} $path" . (isset($this->url_segments['query']) ? '?' . preg_replace('#[^a-zA-Z0-9$\-_.+!*\'(),;/?:@=&]+#e', "urlencode(stripslashes('$0'))", urldecode($this->url_segments['query'])) : '') . " HTTP/1.1\r\n";
...
//		$headers .= "Accept-Encoding: gzip, deflate\r\n";
		...
		$cookies = implode('; ', array($cookies, $this->proxy_cookies));
        if ($cookies != '')
        {
    ...
    function set_cookies()
    {
        if (preg_match_all("#set-cookie:([^\r\n]*)#i", $this->response_headers, $matches))
        {
            foreach ($matches[1] as $cookie_info)
            {
				...
				*/
				$this->all_cookies["COOKIE;$name;$domain;$path"] = $value;
				$_SESSION['cookies'] = $this->all_cookies;
            }
    ...
    {
        if (!empty($this->all_cookies))
        {
            $cookies = '';

            foreach ($this->all_cookies as $cookie_name => $cookie_value)
            {
	...
	*/
    ...
    function return_response($send_headers = true)
    {
		...
		$f_parts = explode('.',$this->base['file']);
		if ($this->content_type == 'text/javascript' || $this->content_type == 'application/x-javascript' || $f_parts[count($f_parts)-1]=='js')
    ...
    function proxify_url($url, $proxify = true)
    {
		...
		if (strlen(preg_replace('#\W#','',$url))==0)
			return '';
		...
		//$add = '?ref='.base64_encode($this->url);
