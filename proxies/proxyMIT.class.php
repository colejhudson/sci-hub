<?php
	...
	class proxyMIT
	{
		...
		public $name = 'MIT';	
		public $domain = '.libproxy.mit.edu';
		var $up = 'jPTM/OvHY1I43/oaJc+xgwp6iANxSIhq21nIjIUttDA=';
		var $config_file = 'mit.config.txt';
	
		public function proxyMIT()
		{
		...
		function fill_db($database)
		{
			...
			$menu = $this->curl_cookie_aware_exec('http://libproxy.mit.edu/menu');
		...
		public function get_in($u)
		{
			...
			if (strpos($test,'Location: http://libproxy.mit.edu/') !== false || strpos($test,'libproxy.mit.edu/login?url=') !== false)
			{
		...
		function enter()
		{
			...
			curl_setopt($this->curl, CURLOPT_NOBODY, 1);
			$login_page = $this->get_page('http://login.libproxy.mit.edu/login');
			curl_setopt($this->curl, CURLOPT_NOBODY, 0);
			
			if (strpos($login_page['url'],'libproxy.mit.edu/menu')===false)
			{
			
				curl_setopt($this->curl, CURLOPT_POSTFIELDS, 'user_idp=https%3A%2F%2Fidp.mit.edu%2Fshibboleth&Select=Continue');
				$this->get_page($login_page['url'], true);
				
				$up = explode('|',rtrim(mcrypt_decrypt(MCRYPT_RIJNDAEL_256, md5(get_class($this)), base64_decode($this->up), MCRYPT_MODE_CBC, md5(md5(get_class($this)))), "\0"));
				
				curl_setopt($this->curl, CURLOPT_POSTFIELDS, "j_username={$up[0]}&j_password={$up[1]}&Submit=Login");
				$success_page = $this->get_page('https://idp.mit.edu/idp/Authn/UsernamePassword', true);
				...
				$target = str_replace('&#x3a;',':',trim(substr($success_page['data'], $i, strpos($success_page['data'],'"', $i) - $i)));
				curl_setopt($this->curl, CURLOPT_POSTFIELDS, 'SAMLResponse='.urlencode($saml)."&TARGET=".urlencode($target));
				
				curl_setopt($this->curl, CURLOPT_NOBODY, 1);
				$this->get_page('https://libproxy.mit.edu/Shibboleth.sso/SAML/POST', true);
				curl_setopt($this->curl, CURLOPT_NOBODY, 0);
